...
# command line parameters
    location = Location.new(:parent_id => nil,
                            :x => nil,
                            :y => nil,
                            :z => nil)
    ...
    opts = OptionParser.new do |opts|
      ...
      opts.on("-s", "--save", "save location specified by id")do
        request_target = :save
      end
    ...
    if request_target.nil? || location.id.nil? ||
       request_target == :update && location.x.nil? && location.y.nil? && location.z.nil? && 
                                    location.parent_id.nil? && movement_strategy_type.nil? && movement_strategy_encoded.nil?
       puts opts
       puts "must specify a command to perform, a location id, and other required options"
       exit
    end


    client = Client.new({:request_target => request_target, :location => location, :movement_stratgy_type => movement_strategy_type})
    client.join if request_target == :update
end
